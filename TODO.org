...
*** DONE Minimal complexity pattern
...
*** TODO Rotate 
  ...
  Note another level of control:  order of motifs
  in original lists, from repeating the same order
  to repeating a random order, maybe with rotation
  successively by one in the middle.
 
  But the idea is for 'N' elements, pick the same
  permuation for all of them.  Which would mean 
  taking an index list, permuting it, and using 
  the same index list to order each of the original
  motif lists.

  What sort of data structure?  Could just be list
  of list with contents varying by motif type:  interval,
  duration, accent, dynamic.  Translations would be to
  map into a sequence e.g. of just repetitions of the
  original order (i.e. concatenation), to map into a
  sequence by a consistent permutation (e.g. rotation)
  successively, to map into a sequence randomly, but
  so that all components follow the same sequence.

  Inputs are:
    1) sequence, which is list of a list of motif,
    2) function that acts on the sequence to vary it,
       type list of list of motif to list of list of 
       motif
    3) count of times to apply the function

  Problem is to capture progress so you can do something
  more than just repeat the exact same operation on the
  list of motifs every time.

  If input is anything other than the source material (motif)
  then that means carrying through some sort of state, or using
  a fold.  So for example, to rotate successively by 1, you'd
  need ... really, only the previous result ... so that could 
  just be a scan.
