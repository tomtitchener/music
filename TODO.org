...
*** TODO Dynamic Clef
Arpeggio example shows need to change clef.  But don't want frequent changes
in case of only a short stretch of notes that exceeds say two leger lines above
or below.  
What if I were to compute the static list of available clefs given any 
clefs currently in NonEmpty VoiceEvent and the starting clef given the 
first 50 notes?  
Then as my state I passed in (Clef, VoiceEvent Clef :| []) as the kernel
and NonEmpty VoiceEvent as the list to fold over?
That wouldn't be enough.  I'd need some running computation of the average
pitch or something that told me when to switch clefs.  Maybe that would
replace the Clef in the first element of the pair so it'd look like
((Pitch,Octave),NonEmpty VoiceEvent) instead.  Except that doesn't answer
because (Pitch,Octave) are to discrete.  Goal is a running average on the
subset of data to compute the running average over.  That begins to look
like ([(Pitch,Octave)],NonEmpty VoiceEvent) where on each iteration I 
derived the running average from the [(Pitch,Octave)] and swapped in the
new (Pitch,Octave) and swapped out the oldest.  What does it look like 
to take the average of a list of (Pitch,Octave)?  The answer would be to
normalize them to e.g. an integer from lowest to highest pitch, then take
the average of that over the list, and then convert it back to (Pitch,Octave)
(which I think I already have routines to do).  Which means I could save
computes by eagerly normalizing and using ([Int], NonEmpty VoiceEvent
So this would be an edge-triggered change:  the instant the next Note 
pulled the average beyond the range of the current Clef, I'd add a Clef
VoiceEvent and change the current clef for the next computation.  Then
the problem would be bouncing between clefs on a passage that danced around
the dividing point between them.  So maybe at that point it'd make sense 
to purge the [Int] record and start from scratch, where we'd recognize a
buffer that wasn't full yet by just adding to the list and passing the
VoiceEvent through unchanged.  Except note that to tell when my average
exceeded the boundaries of the current clef, then I'd need to know what
those boundaries were.  So that clef needs to be part of the state, e.g.
(Clef, [Int], NonEmpty VoiceEvent)

So now the plan is:
  A) Pre-compute for running-average buffer of length N.  The first is
     a constant referenced by scope in the fold that follows.  The rest
     are are inputs to the fold.
     1) List of clefs to choose from based on any VoiceEvent Clef in
        existing stream and default list of four, sorted by register.
     2) Starting clef by sampling first N VoiceEvent Note, taking the
        average of normalized (Pitch,Octatve) pairs and mapping that 
        back to list of ranges for list of clefs.  Note there may be
        multiple clefs that cover the range so I need some measure
        of which clef the average pitch fits best.  Which can be
        ratio of distance to top over distance to bottom and best
        is ratio closest to 1.
     3) NonEmpty VoiceEvent with all VoiceEvent Clef stripped.
  B) fold function that takes (Clef, [Int], NonEmpty VoiceEvent) where
     values will be (startingClef, [], VoiceEvent startingClef .| []) 
     and answers same.
     foldfun :: VoiceEvent -> (Clef, [Int], NonEmpty VoiceEvent) -> (Clef, [Int], NonEmpty VoiceEvent)
     Behavior first by type of VoiceEvent, then by state of buffer 
       - ve@(VoiceEvent (Note ...))
         if length buf < N
         then 
           (clef, norm pit oct:buf, ves <> [ve]) -- buf is [newest -> oldest]
         else 
	   (clef', buf'', ves <> ves')
           where
             buf' = norm pit oct : take (N - 1) buf
             clef' = if avg buf' < low_range clef then next_down clef cleffs 
                     else if avg buf' > high_range clef then next_up clef cleffs
                     else clef
             ves' = if clef' == clef then [ve] else [VoiceEvent clef', ve]
             buf'' = if clef' == clef then buf' else []
       - (VoiceEvent Clef) -> (clef, buf, ves)
       - ve -> (clef, buf, ves <> [ve])
