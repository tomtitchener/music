...
** DONE Fill-in/Fade-out  
  ...
  Reverse to fade-out.
** DONE For a section, extend all voices to equal the longest
Currently I just fill the remainder out with rests.  
But if I used the same logic to compute the duration remaining to the
length of the longest voice, then I could generate stretches for just 
the duration using the last -- or I suppose, the current configuration
if I make this into a section mod or maybe just a new type of voice 
mod -- then the question is how to drive the clipping?  Not all voice
types work by clipping to a given duration.  Instead, maybe it'd make
more sense to keep iterating on the voice until the length exceeds
the amount needed for that voice and then clip that.  The thing is,
the current mod types work one time from Config -> Config or from 
[VoiceEvent] -> [VoiceEvent].  And they work by the individual voice,
though you can determine which voice by the runtime info.
I'm back at need to propagate behavior X over a list of voices based
on some global state Y.  In this case Y is the duration of the longest
voice.  This is all made more complicated by the fact that I only do
the stitching together of lists of voices in the main loop, cfg2Score,
which is where I can determine the accumulated lengths of all voices
for the first time.  All the existing processing is within the scope
of a section only.  
So maybe what I'm really discovering is the need to have a super section
type section where I can play around with this sort of tidying operation.
Or just a higher-level grouping of sections into a ... group?  And it'd
be at the level of the group that I could include an attribute that says
to extend all voices to match the length of the longest voice by repeating
the last section, and the main loop would accumulate groups instead of
sections.
What I'm looking at here is really how to bridge to a transition section.
For the bridge sketch I want to have all voices catch up to each other so
I can start the group that follows at the same point.  Which might even 
saying something like extend all voices to the start of the next bar 
after the end of the current longest-voice. 
** DONE Config mod to gradually expand durations
Effect should be to incrementally slow down rhythms.  Maybe voice-by-voice
with each repetition?  Could double durations to cut tempo in half immediately.
Or could blend between two textures voice-by-voice.  So first would have
fast rhythms of first and fourth voices, second slow rhythms of second and
third voices.  The nature of this group would be to blend together successive
sections after first repeating the count of repetitions for that section.
See doubleDurs keyword, dblCfgMod for modulus for staggering voices.  A 
dblCfgMod of 0 says don't stagger them at all. 
** DONE Add attribute to extend to end of bar
Should work as part of equalEnds maybe, or arbitrarily?
Goal is to get section to start at known location within the
bar. 
This turned out to be much more work than I expected.  For one thing,
I was doing a foldr on new voices for the last seg to fill in the space
to match the longest voice which reversed the previous config, in this 
case making a transpose that ascended instead of descending.  For another,
my annotations to indicate the beginning of a new section were skipping 
voice events that consumed durations like rests and chords to annotate
the first note, so I mistakenly thought my computation of the durations
was wrong when it was actually the position of the annotation.  
** DONE Lilypond command-line to ignore warnings:  lilypond --loglevel=ERROR bridge.ly
** DONE Investigate the problem with triplets.
I must be doing something wrong.  Except I can't figure out what it might
be from the documentation and I can't figure out how to query for the answer here:
  https://lists.endsoftwarepatents.org/archive/html/lilypond-user/2016-06/msg00434.html
Note I tried adding spaces around the list of notes but it didn't make any difference.
I just can't get legitimate tuplet notation for my simple, 3/2 eighth-note tuplets in
7/8 time.  I tried removing the special formatting in the \score directive and that
didn't make any difference except it ruined one double-page where the music on the RHS
was clipped apparently running past the border there.
But then after lettin git rest several days I just tried doubling the unit and everything
rendered as expected.  But all my math has to be redone though.  Or else I just have to 
double the unit when I convert to Lilypond I guess.
** DONE Framework for controls like sustainOn/sustainOff
Experimenting by manually editing lilypond files shows this renders rather nicely
in midi.  Question is how to fit it into the config file.  In terms of the type 
hierarchy, maybe it could be another instance of VoiceEvent:  VoiceEventControl,
where there'd be different sorts of controls.  Or more properly, I should generalize
a note attribute, add instances for dynamic and accent, and include sustainOn and
sustainOff as well, and replace the separate items in Note and Rhythm with a list of
Note Attributes?  Seems more flexible, though I might want to then limit use of the
Note type in aggregates like Chord and Tremolo, though in fact that isn't so bad as
Chord already just has NonEmpty (Pitch,Octave), with individual NonEmpty Accent, Dynamic,
and Swell.  
As a second topic, why is it NonEmpty Accent anyway?  I guess it's allowed in LilyPond,
so thats good enough.  But the real reason is because I wanted a regular, detache effect
via the uniformAccs norrsmods (which should really be vemsods).  So that means e.g. a
staccatissimo for articulation along with an accent for emphasis.
As it is, I allow--and so does Lilypond--for multiple articulations, e.g. staccatissimo
AND accent, which intuitively makes sense to me as different variables, note duration vs.
note weight relative to the surrounding notes--in effect a temporary override on the 
prevailing dynamic.  And Lilypond does render a difference in the midi output, though the
accent effect is really audible in the quietest dynamics only.
But the point was to have a secco articulation as a background, which certainly clutters
the actual notation unnecessarily.  The proper solution to this is to have two versions 
of the Lilypond output, one for the score, one for midi.  For which you can also use tags
to annotate a single Lilypond stream for either destination (\tag #'midi, with alternate
\tag #'layout, together with \removeWithTag #'midi | #'layout for the different targets).
The real way to do this would be to have a global-scope "sempre" category that was 
intepreted to mean a constant stream of annotation per note or rhythm where the printed
score showed just the "sempre staccatissimo" notation but the midi had the attribute 
attached to each note via a tag (see "Try midi rendering with arpeggios").
But this seems like only the sort of pain I'd want to go through if I ever get the 
chance to put a score before actual performers, which is pretty remote, so leave it 
for later, and keep idea of multiple articulations per note to allow for both accent 
and articulation together.
In practice, this took some experimentation.  The document shows:
...
But to get this to work I had to rearrange things slightly:
...
And presumably similarly for dynamic, swell, and sustain.
Probably it'd be easiest to have two lists of attributes, one per tag?  Or maybe just a
Maybe NonEmpty Control for midi only attributes, seeing as there are a few more:  pan/balance,
expression level, reverb, chorus.
Also, with the \articulate script, which I do invoke, you get "slurs, including phrasing slurs"
which could be appealing for wind instruments.

So I turn NonEmpty Accent, Dynamic, and Swell into NonEmpty Attribute sum type with
instances for each together with the new Sustain attribute with instances SustainOn 
and SustainOff.

And then I have to figure out from a generative sense how to slip those sustain attributes
into the event stream.  For my current piece, the best strategy will probably just be a
vesmod to tag the first so sound in a section with the sustain on attribute, maybe to add
a sustainOff attribute to the last note, too.  In the long term, it seems likely there'll
be sustainOn/sustainOff pairs that bracket a handful of notes that'll have to be part of 
the configuration somehow, or else implicit in the generation of the piece by title or
voice type or something else.

Consider more generally how to manage midi-only controls like uniform accents vs. notated
controls like dynamics.  The real problem is that an annotation like "sempre staccatissimo"
doesn't cause any midi effect.  Whereas if you set a dynamic then that leaves the dynamic
in effect for all the notes that follow until the next dynamic.

In fact, *is* there any other midi-only effect I have in mind?  It's sort of fun to imagine
fixing the pan settings in the configuration, but doesn't seem worth the effort unless I'm
going to do somethign funky like swap them around during the course of a piece, which seems
unlikely.  Then there's echo and reverb that seems like it'd be better to manage in Logic.

So in effect what I'm saying is a mod like uniformAccs is the *only place* I need to 
generate both an annotation for the score (and midi too, which just ignores it) and a
midi-tagged dynamic per note.

Neither does it seem to matter what the order of attributes is, e.g. I can put an accent '>'
before the staccatissimo and it renders fine.  And the ordering doesn't seem to matter.

Note that for some controls, it really makes no sense to have multiples per note, which argues
in favor of e.g. a unique dynamic.  But on the other hand, few notes require dynamic as it's
sticky, which argues in favor of a list where than can easily be none.  Also I think there's a
fair amount of boilerplate where I have to initialize a dynamic member with NoDynamic, which is
stupid.

And similarly for NoSwell and NoAccent.

So the question is do I really want a flag *per-control* to indicate midi-only when the only
current use-case is for accent, though it may be in the future I want something like the voice
controller, maybe.  Doesn't seem like it.  Seems smarter to have two lists of controls, one 
for the score and one for midi, or maybe just e.g. _noteControls and _noteMidiControls and ditto
for Rhythm and Chord.  

Then my implementation of uniformAccs can change to show a "sempre" annotation on the first
note of the first segment and a single-item list of midi controls with just a CtrlAccent element,
and maybe I can always render the midi controls first, if only for consistency.  But wait a 
second, I certainly don't want NonEmpty for the list of midi controls, which almost always *be*
empty.  And probably I don't want NonEmpty for the ordinary controls, either.

** TODO Control complexity for mix of durations, pitches, and accents
For any voice type,  have a way to tie one or more voices together through
sharing of the same configuration parameter.
The sharing should take the form of equal access to the same stream of random
choices of individual elements with a list of a list of elements between one 
or more voices:  a dup of a stream among the target voices.
This means lifting the stream generation from the voice to the section and
duping the stream to multiple voices.
Or maybe I can add this as a driver method where each call gets a list of 'n'
results that are the same.
Consider the context:  sectionConfig2VoiceEvents.  Each is currently a traversal
over a list of tuples where one element is the individual VoiceConfig, which is 
where the params are located:  accentss, durationss, pitchess.  Those config params
get passed on to voiceConfig2VoiceEvents where we interact with the driver to get
the list of random accents, durations, and pitches that generate the notes.  
Now what I'm saying is I want a way to hoist one or more configuration elements 
from a given voice, use that to run the driver command myself or provide a callback
to do that where I dupliate the return across as each target voice, and plumb that
back into the voice generation process, maybe as a runtime config param.
Could this be a config mod where e.g. the list of list of durations got turned into
a single item list shared among the target voices?  
The question is, if there were multiple segments for the section, would the count of
elements in the list stay synchronized between voices between segments?
For durations, you could manage the number of list items consumed by limiting the 
overall duration for the section.  Note currently the duration limit is part of the
voice configuration, so I'd have to be careful the voices in question just had the
same value.  Also notice the transposing voice doesn't have a duration limit, though
I suppose it could have either that or a transposition limit, though that's going to
make for an awkward implementation.
Ideally there'd be a way to generate the list of e.g. durations and then have it 
available to pass into the relevant voices as a runtime parameter that the voice 
would somehow know to inspect for and use if available.  
What about the other two parameters:  pitch,octave pairs and accents?  Am I likely
to ever want to hold the generation of those constant independently or in parallel
with durations?
Note that it'd be possible to explore these effects without any extra coding using
the current setup just by cloning between voices and e.g. keeping the duration the
same as well.
Maybe before I do any coding it'd be worthwhile seeing if this is even an effect
that's promissing to explore or if maybe it'd just be good enough to do it via the
config file.
Except that wouldn't really work either, as there'd be no variation between sections
or within repeats of a section.
Or maybe not.
It may be that capturing complexity like this isn't really feasible in the context
of the records and routines I have so far, and I need to rethink e.g. an association
of group/section/voice configurations that targets the possibility of replicated 
configuration data.
** TODO group config that blends section to section.
Take some count of first section by config, then 
progressively blend in voice-by-voice from next section,
and etc. for all successive sections.
Have a group config param to say how many repetitions of
solid blend before starting transition with minimum of 1.
Consider blend possibilities: 
  * voice-by-voice, e.g. generate the [ [VoiceEvent] ] first, then 
    interleave voices, could get confusing if the different sections
    have different or just non-fixed lengths
  * configuration element by configuration element -- this wouldn't work
    for transpose voice configs, but you could swap among the list of list of
    e.g. mPitOctOrPitOcts or DurOrDurTuplets, chosing sub-list by sub-list,
    though you wouldn't know how many sublists would be involved, and results    
    interleaved with other section might be a little bizarre
    could also do this horizontally for all voices at once, to make a sort
    of holographic migration
Having trouble trying to force this.  Possible to add a cell voice with 
config to force repeated patterns with regular durations.  But it's never
quite what I had in mind, or somehow lacking.  And it seems like a  lot of
work for the result.
What I've had more success with in terms of experimental composing, is to 
work on some sort of combinator where I merge two textures.  In this model,
the final merge has all voices operating from the new config as one texture
gradually merges into a second.  
** TODO Accents to reflect meter
So far it's all chaos, without respect to the bar, could be any time signature.
What shapes a sense of meter?  Not just accents, but groupings in melodies as well.
But this is, so far, a type of non-melodic music, and I like it like that.  And
it's also a non-melodic music without a monotonic pulse or even a regular period
of repetition.
** TODO Voice mod for custom dynamics.
It'd be nice to have a way to rewrite e.g. dynamics, say for when
even-ends repeats the last section picking up dynmaics from the
beginning but I want to tailor the dynamics to what comes next.
** TODO Try midi rendering of arpeggios.
Doesn't work.  See workaround:
https://www.mail-archive.com/search?l=lilypond-devel@gnu.org&q=subject:%22Re%5C%3A+A+question+of+rendering+multiple+instances+of+%5C%5Carpeggio+in+MIDI%22&o=newest&f=1
Or better yet:
https://www.mail-archive.com/lilypond-user@gnu.org/msg142903.html
Trick is to insert tags separating pdf from midi output, with special hacks to midi to render the arpeggio as a roll.
And probably to update the Scheme code to match the version of Lilypond.
** TODO Annotate variable names for inifinite lists with "inf" tag.
